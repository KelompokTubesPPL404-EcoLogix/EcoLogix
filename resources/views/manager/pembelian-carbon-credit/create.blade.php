                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('manager.pembelian-carbon-credit.store') }}" enctype="multipart/form-data">
                            ...
                            <div class="col-md-9">
                                <div class="input-group">
                                    ...
                                    <select id="kode_kompensasi" class="form-select @error('kode_kompensasi') is-invalid @enderror" name="kode_kompensasi" required>
                                        <option value="">-- Pilih Kode Kompensasi --</option>                                         @foreach($kompensasiList as $kompensasi)
                        ...
                        <div class="mb-4 row">
                            ...
                            </label>                            <div class="col-md-9">                                <div class="input-group">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Debug elemen DOM yang diambil untuk memastikan tidak ada yang null
    ...
    function hitungTotalHarga() {
        ...
        // Set nilai hidden inputs untuk form submission
        totalHargaInput.value = total.toFixed(2);
        
        // Update display untuk total harga
        ...
        // Debug log untuk memastikan nilai tersimpan
        console.log('Hidden input values set to:', {
    ...
    kompensasiSelect.addEventListener('change', function() {
        ...
        jumlahInput.value = jumlah;  // Mengisi hidden field jumlah_kompensasi
        hitungTotalHarga();
        if (this.selectedIndex > 0) {
            // Menggunakan dataset untuk mengakses data-attributes
            const jumlah = selectedOption.dataset.jumlah || '0';
            
            console.log('Selected kompensasi:', {
                option: selectedOption.text,
                jumlah: jumlah
            });
            
            // SIMPAN NILAI KE INPUT TERSEMBUNYI - SANGAT PENTING!
            jumlahInput.value = jumlah;
            
            // Hitung total
            hitungTotalHarga();
            
            // Debug: Tampilkan nilai setelah diisi
            console.log('After setting value:', {
                hidden_input: jumlahInput.value,
                display_input: jumlahDisplay.value
            });
        } else {
    ...
    penyediaSelect.addEventListener('change', function() {
        ...
        if (this.selectedIndex > 0) {
            ...
            // Debug: Tampilkan nilai setelah diisi
            console.log('After setting penyedia value:', {
                harga_per_ton: hargaPerTonInput.value,
                mata_uang: mataUangDisplay.textContent
            });
        } else {
            // Reset nilai jika tidak ada yang dipilih
        ...
        // Hapus delayed initialization karena tidak ada di file referensi// Verifikasi sebelum submit - lebih sederhana sesuai referensi
    document.querySelector('form').addEventListener('submit', function(event) {
        // Log nilai sebelum submit
        ...
        // Hitung ulang total harga untuk memastikan nilai benar
        const jumlahKg = parseFloat(jumlahInput.value) || 0;
        const hargaPerTon = parseFloat(hargaPerTonInput.value) || 0;
        const jumlahTon = jumlahKg / 1000;
        const total = jumlahTon * hargaPerTon;
    ...
    if (refreshButton) {
        refreshButton.addEventListener('click', function() {
            ...
            // Trigger change events untuk memperbarui nilai
            if (kompensasiSelect.selectedIndex > 0) {
                kompensasiSelect.dispatchEvent(new Event('change'));
            }
            
            if (penyediaSelect.selectedIndex > 0) {
                penyediaSelect.dispatchEvent(new Event('change'));
            }
            
            // Hitung total harga
            hitungTotalHarga();
            
            // Verifikasi nilai hidden inputs
            console.log('Hidden inputs after refresh:', {
                jumlah_kompensasi: jumlahInput.value,
                harga_per_ton: hargaPerTonInput.value,
                total_harga: totalHargaInput.value
            });
